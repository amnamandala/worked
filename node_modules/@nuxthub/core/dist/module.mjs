...
async function setupAI(nuxt, hub) {
  if (nuxt.options.dev && !hub.remote && !hub.projectKey && (!hub.cloudflare?.accountId || !hub.cloudflare?.apiToken)) {
    log$2.warn("`hubAI()` and `hubAutoRAG()` are disabled in development mode: set `NUXT_HUB_CLOUDFLARE_ACCOUNT_ID` and `NUXT_HUB_CLOUDFLARE_API_TOKEN` environment variables to run AI models in development mode.");
    log$2.info(`Create a Cloudflare API token with required permissions using \`https://hub.nuxt.com/cloudflare-token\``);
    return;
  }
...
async function setupRemote(_nuxt, hub) {
  ...
  if (hub.projectKey) {
    ...
    log$2.warn("NuxtHub Admin is being sunset on 31st December 2025. To continue using remote storage, please switch to self hosting. Learn more at https://hub.nuxt.com/docs/getting-started/remote-storage#local-development");
  ...
  if (!hub.projectUrl) {
    log$2.error("No project URL defined, make sure to set the `hub.projectUrl` option in your `nuxt.config.ts` file or add the deployed URL as `NUXT_HUB_PROJECT_URL` environment variable.");
    process.exit(1);
  }
  ...
  if (hub.projectUrl && hub.userToken && !hub.projectSecretKey) {
    log$2.warn("NuxtHub Admin is being sunset on 31st December 2025. To continue using remote storage, please switch to self hosting and set the `NUXT_HUB_PROJECT_SECRET_KEY` environment variable. Learn more at https://hub.nuxt.com/docs/getting-started/remote-storage#local-development");
  }
